...
{
    class PrayStar : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            ProjectileID.Sets.TrailCacheLength[Projectile.type] = 4; // 拖尾缓存长度
        ...
        {           
            ...
            {
                starSpeed = Main.rand.NextFloat(10f, 15f);
            }
            // 前减速
            ...
            else // 开始追踪敌人
            {
                ...
                // 获取目标 NPC
                NPC target = Projectile.Center.ClosestNPCAt(1500f, true);
                ...
                if (target != null && target.active)
                {
                    ...
                    // 设置弹幕的最大转向速度（弧度/帧）
                    float maxTurnSpeed = 0.05f + 0.5f*(distanceToTarget/1500f);
                    ...
                    // 使用向量插值逐渐调整速度方向
                    Vector2 newDirection = Vector2.Lerp(currentVelocity, direction, maxTurnSpeed);
                    newDirection.Normalize();

                    // 设置弹幕的新速度
                    Projectile.velocity = newDirection * starSpeed;
                }
            }

            // 添加光照
            Lighting.AddLight(Projectile.Center, 0.5f, 0.0f, 1.0f); // 深紫色光
